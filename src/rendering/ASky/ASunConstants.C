 ...
 *	
 * 
 * $Id: ASunConstants.C,v 1.2 1999/07/15 00:07:33 bes Exp $ 
 * 
 */
#ifndef ACOMMON_H
//#include <ACommon.H>
...
static AReal k_oWavelengths[64] = {
300, 305, 310, 315, 320,
325, 330, 335, 340, 345,
350, 355,
...
static AReal k_waWavelengths[13] = {
  689,
  690,
  700,
  710,
...
static AReal k_waAmplitudes[13] = {
  0,
  0.160e-1,
  0.240e-1,
  0.125e-1,
...
ASpectrum ASunSky::ComputeAttenuatedSunlight(AReal theta, int turbidity)
{
    AIrregularSpectralCurve k_oCurve(k_oAmplitudes, k_oWavelengths, 64);
    AIrregularSpectralCurve k_gCurve(k_gAmplitudes, k_gWavelengths, 4);
    AIrregularSpectralCurve k_waCurve(k_waAmplitudes, k_waWavelengths, 13);
    ARegularSpectralCurve   solCurve(solAmplitudes, 380, 750, 38);  // every 10 nm  IN WRONG UNITS
				                                     // Need a factor of 100 (done below)
    AReal  data[91];  // (800 - 350) / 5  + 1

    AReal beta = 0.04608365822050 * turbidity - 0.04586025928522;
    AReal tauR, tauA, tauO, tauG, tauWA;

    AReal m = 1.0/(cos(theta) + 0.15*pow(93.885-theta/APi*180.0,-1.253));  // Relative Optical Mass
				// equivalent  
//    AReal m = 1.0/(cos(theta) + 0.000940 * pow(1.6386 - theta,-1.253));  // Relative Optical Mass
    ...
    return ARegularSpectralCurve(data, 350,800,91);  // Converts to ASpectrum
}

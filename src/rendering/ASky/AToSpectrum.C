 ...
 *	
 * 
 * $Id: AToSpectrum.C,v 1.1 1999/06/10 20:12:34 bes Exp $ 
 * 
 */
#ifndef ACOMMON_H
//#include <ACommon.H>
...
static ASpectrum riS0Spectrum, riS1Spectrum, riS2Spectrum;

void InitChromaticityToSpectrum()
{
    riS0Spectrum = ARegularSpectralCurve(S0Amplitudes,300,830,54);
    riS1Spectrum = ARegularSpectralCurve(S1Amplitudes,300,830,54);
    riS2Spectrum = ARegularSpectralCurve(S2Amplitudes,300,830,54);
}

ASpectrum ASunSky::ChromaticityToSpectrum(AReal x, AReal y) const
{
    ...
    AReal M1 = (-1.3515 - 1.7703*x +  5.9114*y)/(0.0241 + 0.2562*x - 0.7341*y);
    AReal M2 = ( 0.03   -31.4424*x + 30.0717*y)/(0.0241 + 0.2562*x - 0.7341*y);
